...
def _serve(app: Flask, host: str, port: int) -> None:
    ...
    print(f"[web] waitress serving on http://{host}:{port}",
          file=sys.stderr)
    # One uploaded MP4 == one request body; tournament files run to several GB,
    # well past waitress's 1 GB default (which rejects the chunk with HTTP 413).
    # Allow large bodies and give a slow large-file upload room before the
    # channel idle-timeout fires.
    waitress_serve(
